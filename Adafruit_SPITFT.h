...
// HARDWARE CONFIG ---------------------------------------------------------
	...
	typedef uint32_t		ADAGFX_PORT_t;		///< PORT values are 32-bit
		...
		// GPIO register set and will require some changes elsewhere (e.g. in constructors especially).
	#endif
#else		// !ARM
	// Probably ESP8266 or ESP32.
	// USE_FAST_PINIO is not available here (yet) but don't worry about it too much...the digitalWrite() implementation
	// on these platforms is reasonably efficient and already RAM-resident, only gotcha then is no parallel connection support for now.
	typedef uint32_t		ADAGFX_PORT_t;		///< PORT values are 32-bit
#endif		// end !ARM
...
// Needed a way to disambiguate the software SPI and parallel constructors via their argument lists.
// Originally tried a bool as the first argument to the parallel constructor (specifying 8-bit vs 16-bit interface) but the compiler regards this as equivalent to an integer and thus still ambiguous.
// SO...the parallel constructor requires an enumerated type as the first argument: tft8 (for 8-bit parallel) or tft16 (for 16-bit)...
// even though 16-bit isn't fully implemented or tested and might never be, still needed that disambiguation from soft SPI.
...
/*!
  @brief	Adafruit_SPITFT is an intermediary class between Adafruit_GFX
		...
		It handles certain operations that are common to a range of displays (address window, area fills, etc.).
		Originally these were all color TFT displays interfaced via SPI, but it's since expanded to include color OLEDs and parallel-interfaced TFTs.
		THE NAME HAS BEEN KEPT TO AVOID BREAKING A LOT OF SUBCLASSES AND EXAMPLE CODE.
		Many of the class member functions similarly live on with names that don't necessarily accurately describe what they're doing, again to avoid breaking a lot of other code.
		If in doubt, read the comments.
*/
class Adafruit_SPITFT : public Adafruit_GFX {
 public:
	// CONSTRUCTORS --------------------------------------------------------

	// Software SPI constructor: expects width & height (at default rotation setting 0), 4 signal pins (cs, dc, mosi, sclk), 2 optional pins (reset, miso).
	// cs argument is required but can be -1 if unused -- rather than moving it to the optional arguments, it was done this way to avoid breaking existing code (-1 option was a later addition).
	Adafruit_SPITFT(uint16_t w, uint16_t h, int8_t cs, int8_t dc, int8_t mosi, int8_t sck, int8_t rst = -1, int8_t miso = -1);

	// Hardware SPI constructor using the default SPI port: expects width & height (at default rotation setting 0), 2 signal pins (cs, dc), optional reset pin.
	// cs is required but can be -1 if unused -- rather than moving it to the optional arguments, it was done this way to avoid breaking existing code (-1 option was a later addition).
	Adafruit_SPITFT(uint16_t w, uint16_t h, int8_t cs, int8_t dc, int8_t rst = -1);

#if !defined(ESP8266)		// See notes in .cpp
	// Hardware SPI constructor using an arbitrary SPI peripheral: expects width & height (rotation 0), SPIClass pointer, 2 signal pins (cs, dc) and optional reset pin.
	// cs is required but can be -1 if unused.
	Adafruit_SPITFT(uint16_t w, uint16_t h, SPIClass *spiClass, int8_t cs, int8_t dc, int8_t rst = -1);
#endif		// end !ESP8266

	// Parallel constructor: expects width & height (rotation 0), flag indicating whether 16-bit (true) or 8-bit (false) interface, 3 signal pins (d0, wr, dc), 3 optional pins (cs, rst, rd).
	// 16-bit parallel isn't even fully implemented but the 'wide' flag was added as a required argument to avoid ambiguity with other constructors.
	...
	*/
	...
	/*!
	@brief  Set up the specific display hardware's "address window" for subsequent pixel-pushing operations.
	@param  x  Leftmost pixel of area to be drawn (MUST be within display bounds at current rotation setting).
	@param  y  Topmost pixel of area to be drawn (MUST be within display bounds at current rotation setting).
	@param  w  Width of area to be drawn, in pixels (MUST be >0 and, added to x, within display bounds at current rotation).
	@param  h  Height of area to be drawn, in pixels (MUST be >0 and, added to x, within display bounds at current rotation).
	*/
	virtual void	setAddrWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h) = 0;

	// Remaining functions do not need to be declared in subclasses unless they wish to provide hardware-specific optimizations.
	// Brief comments here...documented more thoroughly in .cpp file.

	// Subclass' begin() function invokes this to initialize hardware.
	// freq=0 to use default SPI speed. spiMode must be one of the SPI_MODEn values defined in SPI.h, which are NOT the same as 0 for SPI_MODE0, 1 for SPI_MODE1, etc...
	// use ONLY the SPI_MODEn defines! Only!
	...
	// These functions require a chip-select and/or SPI transaction around them.
	// Higher-level graphics primitives might start a single transaction and then make multiple calls to these functions (e.g. circle or text rendering might make repeated lines or rects)
	// before ending the transaction. It's more efficient than starting a transaction every time.
	...
	// This is a new function, similar to writeFillRect() except that all arguments MUST be onscreen, sorted and clipped.
	// If higher-level primitives can handle their own sorting/clipping, it avoids repeating such operations in the low-level code, making it potentially faster.
	...
	// Another new function, companion to the new non-blocking writePixels() variant.
	void		dmaWait(void);

	// These functions are similar to the 'write' functions above, but with a chip-select and/or SPI transaction built-in.
	// They're typically used solo -- that is, as graphics primitives in themselves, not invoked by higher-level primitives (which should use the functions above).
	void		drawPixel(int16_t x, int16_t y, uint16_t color);
	void		fillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color);
	void		drawFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color);
	void		drawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color);

	// A single-pixel push encapsulated in a transaction. I don't think this is used anymore (BMP demos might've used it?) but is provided for backward compatibility, consider it deprecated:
	...
	// Most of these low-level functions were formerly macros in Adafruit_SPITFT_Macros.h.
	// Some have been made into inline functions to avoid macro mishaps.
	// Despite the addition of code for a parallel display interface, the names have been kept for backward compatibility (some subclasses may be invoking these):
	void		SPI_WRITE16(uint16_t w);		// Not inline
	void		SPI_WRITE32(uint32_t l);		// Not inline

	// Old code had both a spiWrite16() function and SPI_WRITE16 macro in addition to the SPI_WRITE32 macro.
	// The latter two have been made into functions here, and spiWrite16() removed (use SPI_WRITE16() instead).
	// It looks like most subclasses had gotten comfortable with SPI_WRITE16 and SPI_WRITE32 anyway so those names were kept rather than the less-obnoxious camelcase variants, oh well.

	// Placing these functions entirely in the class definition inlines them implicitly them while allowing their use in other code:

	/*!
	@brief  Set the chip-select line HIGH. Does NOT check whether CS pin is set (>=0), that should be handled in calling function.
		Despite function name, this is used even if the display connection is parallel.
	*/
	...
	/*!
	@brief  Set the chip-select line LOW. Does NOT check whether CS pin is set (>=0), that should be handled in calling function.
		Despite function name, this is used even if the display connection is parallel.
	*/
 ...
 protected:
	// A few more low-level member functions -- some may have previously been macros.
	// Shouldn't have a need to access these externally, so they've been moved to the protected section.
	// Additionally, they're declared inline here and the code is in the .cpp file, since outside code doesn't need to see these.
	...
	// Here be dragons! There's a big union of three structures here -- one each for hardware SPI, software (bitbang) SPI, and parallel interfaces.
	// This is to save some memory, since a display's connection will be only one of these.
	// The order of some things is a little weird in an attempt to get values to align and pack better in RAM.
